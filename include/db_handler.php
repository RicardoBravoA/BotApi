...
class DbHandler {
    ...
    */
    ...
    public function postMessage($message, $operation, $property_type){
        $response = array();
        $error = true;
        $stmt = $this->conn->prepare("SELECT message_default_id, message, operation, property_type, response_1, response_2, response_type_id FROM message_default WHERE message LIKE '%$message%' AND operation = ? AND property_type = ?");
        $stmt->bind_param("ss", $operation, $property_type);
        if($stmt->execute()){
            ...
            if($stmt->num_rows>0){
                ...
                $data1 = array();
                $data1["response_1"] = "Hola";
                $data1["response_2"] = "¿Qué operación desea realizar?";
                $data1["response_type_id"] = "2";
